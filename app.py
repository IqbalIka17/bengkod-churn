...
<style>
    ...
    .result-label { font-size: 1.6rem; font-weight: 800; margin-bottom: 0.3rem; }
...
with st.sidebar:
    ...
    for _, row in fi_df.head(5).iterrows():
        ...
        st.progress(float(row['Importance']), text=f"{row['Importance']*100:.1f}%")
...
with tab1:
    ...
    if predict_btn:
        ...
        # ── Expander: detail fitur yang dikirim ke model ──────────────────────
        with st.expander("🔎 Lihat detail fitur yang dipakai model", expanded=False):
            fitur_terpakai = {k: v for k, v in input_dict.items() if k in selected_features}
            st.json(fitur_terpakai)
